...
- **Question relay:** if `workflow.yaml` has `intake: questions`, spawn `phase: intake` first (questions only — no `dist`); relay `user_questions` to the user; spawn `phase: execute` with `## Answers (user-confirmed)`. Do not author step questions in the orchestrator skill.
- **Code repos:** set `LAUNCHPAD_DIST` / `LAUNCHPAD_DIST_TYPES` before **execute** (not intake) — see `platform/config/repos.yaml`
